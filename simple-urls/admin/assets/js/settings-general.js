jQuery(document).ready(function () {
    ...
    function save_setting_general(event) {
        ...
        let general_disable_tooltip = jQuery("#general_disable_tooltip").prop(
            "checked"
        )
            ? 1
            : 0;
        let general_disable_amazon_notifications = jQuery(
            "#general_disable_amazon_notifications"
        ).prop("checked")
            ? 1
            : 0;
        let general_disable_notification = jQuery(
            "#general_disable_notification"
        ).prop("checked")
            ? 1
            : 0;
        let general_enable_new_ui = jQuery("#general_enable_new_ui").prop(
            "checked"
        )
            ? 1
            : 0;
        ...
        let performance_event_tracking = jQuery(
            "#performance_event_tracking"
        ).prop("checked")
            ? 1
            : 0;
    ...
    jQuery("#reactivate-lite").click(function () {
        ...
        jQuery
            ...
            .done(function (response) {
                ...
                let empty_license_msg =
                    'Access or create your <a class="purple underline" href="https://app.getlasso.co/account" target="_blank">Lasso account</a> to get your license key.';
                let license_valid =
                    'Your license key is <strong id="is_license_active" class="green">active</strong>. Access your <a class="purple underline" href="https://app.getlasso.co/account" target="_blank">Lasso account</a> to manage installs.';
                let license_invalid =
                    'Your license key is <strong id="is_license_active" class="red">not active</strong>. Access your <a class="purple underline" href="https://app.getlasso.co/account" target="_blank">Lasso account</a> to manage installs.';
            ...
            .fail(function (error, xhr, message) {
                lasso_lite_helper.errorScreen("Failed.");
            })
            .always(function () {
                lasso_lite_helper.set_progress_bar_complete();
                setTimeout(function () {
